...
content = content.replace(
  /const instas = htmlUnificado\.match\(\/instagram\\\.com\\\/\[A-Za-z0-9_\\.\]\+\/gi\) \|\| \[\];/g,
  `const instas = (htmlUnificado.match(/instagram\\.com\\/([A-Za-z0-9_.]+)/gi) || [])${filterRegex};`
);

content = content.replace(
  /const faces = htmlUnificado\.match\(\/facebook\\\.com\\\/\[A-Za-z0-9_\\.\]\+\/gi\) \|\| \[\];/g,
  `const faces = (htmlUnificado.match(/facebook\\.com\\/([A-Za-z0-9_.]+)/gi) || [])${filterRegex};`
);

content = content.replace(
  /const instaMatches = htmlUnificado\.match\(\/instagram\\\.com\\\/\[A-Za-z0-9_\\.\]\+\/gi\) \|\| \[\];/g,
